#!/usr/bin/env node
/**
 * ТЕСТЫ ДЛЯ ЛИСТА РИФЛЕНОГО
 *
 * Проверяем правильность расчёта веса с учётом:
 * - Толщины листа (2.5-12 мм)
 * - Типа рифления (чечевица, ромб)
 */

const { calculateMetal } = require('./src/calculator.js');
const metalDatabase = require('./docs/database/metals.json');

console.log('═══════════════════════════════════════════════════════════════');
console.log('🧪 ТЕСТЫ ДЛЯ ЛИСТА РИФЛЕНОГО');
console.log('═══════════════════════════════════════════════════════════════\n');

// =============================================================================
// ТЕСТ 1: Лист рифленый 5 мм, чечевица, 10 м²
// Ожидаемый вес: 40.5 кг/м² × 10 м² / 1000 = 0.405 т
// =============================================================================
console.log('📋 ТЕСТ 1: Лист рифленый 5 мм, чечевица, 10 м²');
console.log('─────────────────────────────────────────────────────────────\n');

const test1 = calculateMetal({
  metalType: 'sheet_checkered',
  size: '5',
  riffleType: 'чечевица',
  area: 10
}, metalDatabase);

console.log('Параметры:');
console.log('  Толщина: 5 мм');
console.log('  Тип рифления: чечевица');
console.log('  Площадь: 10 м²');
console.log('  Вес 1 м²: 40.5 кг');
console.log();
console.log(`Результат: ${test1.weight.toFixed(3)} т`);
console.log(`Ожидается: 0.405 т`);

const test1Expected = 0.405;
const test1Diff = Math.abs(test1.weight - test1Expected);
const test1Passed = test1Diff < 0.001;

console.log(`📊 Разница: ${(test1Diff * 1000).toFixed(1)} кг`);
console.log(`   Статус: ${test1Passed ? '✅ PASSED' : '❌ FAILED'}`);
console.log();

// =============================================================================
// ТЕСТ 2: Лист рифленый 5 мм, ромб, 10 м²
// Ожидаемый вес: 42.31 кг/м² × 10 м² / 1000 = 0.423 т
// =============================================================================
console.log('📋 ТЕСТ 2: Лист рифленый 5 мм, ромб, 10 м²');
console.log('─────────────────────────────────────────────────────────────\n');

const test2 = calculateMetal({
  metalType: 'sheet_checkered',
  size: '5',
  riffleType: 'ромб',
  area: 10
}, metalDatabase);

console.log('Параметры:');
console.log('  Толщина: 5 мм');
console.log('  Тип рифления: ромб');
console.log('  Площадь: 10 м²');
console.log('  Вес 1 м²: 42.31 кг');
console.log();
console.log(`Результат: ${test2.weight.toFixed(3)} т`);
console.log(`Ожидается: 0.423 т`);

const test2Expected = 0.423;
const test2Diff = Math.abs(test2.weight - test2Expected);
const test2Passed = test2Diff < 0.001;

console.log(`📊 Разница: ${(test2Diff * 1000).toFixed(1)} кг`);
console.log(`   Статус: ${test2Passed ? '✅ PASSED' : '❌ FAILED'}`);

// Проверяем разницу между чечевицей и ромбом
const differenceBetweenTypes = test2.weight - test1.weight;
console.log(`\n📊 Разница ромб - чечевица: ${(differenceBetweenTypes * 1000).toFixed(1)} кг`);
console.log(`   Ожидается: ~18 кг (ромб тяжелее)`);
console.log(`   Статус: ${Math.abs(differenceBetweenTypes * 1000 - 18) < 1 ? '✅ PASSED' : '❌ FAILED'}`);
console.log();

// =============================================================================
// ТЕСТ 3: Лист рифленый 12 мм, ромб, 5 м²
// Ожидаемый вес: 101.13 кг/м² × 5 м² / 1000 = 0.506 т
// =============================================================================
console.log('📋 ТЕСТ 3: Лист рифленый 12 мм, ромб, 5 м²');
console.log('─────────────────────────────────────────────────────────────\n');

const test3 = calculateMetal({
  metalType: 'sheet_checkered',
  size: '12',
  riffleType: 'ромб',
  area: 5
}, metalDatabase);

console.log('Параметры:');
console.log('  Толщина: 12 мм');
console.log('  Тип рифления: ромб');
console.log('  Площадь: 5 м²');
console.log('  Вес 1 м²: 101.13 кг');
console.log();
console.log(`Результат: ${test3.weight.toFixed(3)} т`);
console.log(`Ожидается: 0.506 т`);

const test3Expected = 0.506;
const test3Diff = Math.abs(test3.weight - test3Expected);
const test3Passed = test3Diff < 0.001;

console.log(`📊 Разница: ${(test3Diff * 1000).toFixed(1)} кг`);
console.log(`   Статус: ${test3Passed ? '✅ PASSED' : '❌ FAILED'}`);
console.log();

// =============================================================================
// ТЕСТ 4: Лист рифленый 2.5 мм, чечевица, 20 м²
// Ожидаемый вес: 20.1 кг/м² × 20 м² / 1000 = 0.402 т
// =============================================================================
console.log('📋 ТЕСТ 4: Лист рифленый 2.5 мм, чечевица, 20 м²');
console.log('─────────────────────────────────────────────────────────────\n');

const test4 = calculateMetal({
  metalType: 'sheet_checkered',
  size: '2.5',
  riffleType: 'чечевица',
  area: 20
}, metalDatabase);

console.log('Параметры:');
console.log('  Толщина: 2.5 мм');
console.log('  Тип рифления: чечевица');
console.log('  Площадь: 20 м²');
console.log('  Вес 1 м²: 20.1 кг');
console.log();
console.log(`Результат: ${test4.weight.toFixed(3)} т`);
console.log(`Ожидается: 0.402 т`);

const test4Expected = 0.402;
const test4Diff = Math.abs(test4.weight - test4Expected);
const test4Passed = test4Diff < 0.001;

console.log(`📊 Разница: ${(test4Diff * 1000).toFixed(1)} кг`);
console.log(`   Статус: ${test4Passed ? '✅ PASSED' : '❌ FAILED'}`);
console.log();

// =============================================================================
// ТЕСТ 5: Лист рифленый 8 мм, ромб, 15 м²
// Ожидаемый вес: 67.81 кг/м² × 15 м² / 1000 = 1.017 т
// =============================================================================
console.log('📋 ТЕСТ 5: Лист рифленый 8 мм, ромб, 15 м²');
console.log('─────────────────────────────────────────────────────────────\n');

const test5 = calculateMetal({
  metalType: 'sheet_checkered',
  size: '8',
  riffleType: 'ромб',
  area: 15
}, metalDatabase);

console.log('Параметры:');
console.log('  Толщина: 8 мм');
console.log('  Тип рифления: ромб');
console.log('  Площадь: 15 м²');
console.log('  Вес 1 м²: 67.81 кг');
console.log();
console.log(`Результат: ${test5.weight.toFixed(3)} т`);
console.log(`Ожидается: 1.017 т`);

const test5Expected = 1.017;
const test5Diff = Math.abs(test5.weight - test5Expected);
const test5Passed = test5Diff < 0.001;

console.log(`📊 Разница: ${(test5Diff * 1000).toFixed(1)} кг`);
console.log(`   Статус: ${test5Passed ? '✅ PASSED' : '❌ FAILED'}`);
console.log();

// =============================================================================
// ТЕСТ 6: Проверка всех размеров с чечевицей
// =============================================================================
console.log('📋 ТЕСТ 6: Проверка всех размеров с чечевицей');
console.log('─────────────────────────────────────────────────────────────\n');

const sizes = ['2.5', '3', '4', '5', '6', '8', '10', '12'];
const expectedWeights = {
  '2.5': 20.1,
  '3': 24.2,
  '4': 32.2,
  '5': 40.5,
  '6': 48.5,
  '8': 64.9,
  '10': 80.9,
  '12': 96.8
};

let allSizesPassed = true;

sizes.forEach(size => {
  const result = calculateMetal({
    metalType: 'sheet_checkered',
    size: size,
    riffleType: 'чечевица',
    area: 1
  }, metalDatabase);

  // Ожидаемое значение с учётом округления до 3 знаков после запятой
  const expectedRaw = expectedWeights[size] / 1000; // переводим в тонны
  const expected = Math.round(expectedRaw * 1000) / 1000; // округляем как в калькуляторе
  const diff = Math.abs(result.weight - expected);
  const passed = diff < 0.0001;

  if (!passed) allSizesPassed = false;

  console.log(`  ${size} мм: ${result.weight.toFixed(4)} т (ожидается ${expected.toFixed(4)} т) ${passed ? '✅' : '❌'}`);
});

console.log(`\n📊 Статус: ${allSizesPassed ? '✅ PASSED - все размеры работают правильно' : '❌ FAILED'}`);
console.log();

// =============================================================================
// ТЕСТ 7: Проверка всех размеров с ромбом
// =============================================================================
console.log('📋 ТЕСТ 7: Проверка всех размеров с ромбом');
console.log('─────────────────────────────────────────────────────────────\n');

const expectedWeightsRomb = {
  '2.5': 21.0,
  '3': 25.28,
  '4': 33.64,
  '5': 42.31,
  '6': 50.67,
  '8': 67.81,
  '10': 84.52,
  '12': 101.13
};

let allRombSizesPassed = true;

sizes.forEach(size => {
  const result = calculateMetal({
    metalType: 'sheet_checkered',
    size: size,
    riffleType: 'ромб',
    area: 1
  }, metalDatabase);

  // Ожидаемое значение с учётом округления до 3 знаков после запятой
  const expectedRaw = expectedWeightsRomb[size] / 1000; // переводим в тонны
  const expected = Math.round(expectedRaw * 1000) / 1000; // округляем как в калькуляторе
  const diff = Math.abs(result.weight - expected);
  const passed = diff < 0.0001;

  if (!passed) allRombSizesPassed = false;

  console.log(`  ${size} мм: ${result.weight.toFixed(4)} т (ожидается ${expected.toFixed(4)} т) ${passed ? '✅' : '❌'}`);
});

console.log(`\n📊 Статус: ${allRombSizesPassed ? '✅ PASSED - все размеры работают правильно' : '❌ FAILED'}`);
console.log();

// =============================================================================
// ИТОГИ ТЕСТИРОВАНИЯ
// =============================================================================
console.log('═══════════════════════════════════════════════════════════════');
console.log('📊 ИТОГИ ТЕСТИРОВАНИЯ');
console.log('═══════════════════════════════════════════════════════════════\n');

const allTestsPassed = test1Passed && test2Passed && test3Passed && test4Passed && test5Passed && allSizesPassed && allRombSizesPassed;

if (allTestsPassed) {
  console.log('✅ ВСЕ ТЕСТЫ ПРОЙДЕНЫ УСПЕШНО!\n');
  console.log('🎉 Лист рифленый исправлен:');
  console.log('   1. Вес рассчитывается правильно для всех толщин (2.5-12 мм)');
  console.log('   2. Тип рифления (чечевица/ромб) влияет на вес');
  console.log('   3. Используется правильная формула: вес_1м² × площадь / 1000');
  console.log();
  console.log('✅ Формула работает правильно!');
} else {
  console.log('❌ НЕКОТОРЫЕ ТЕСТЫ НЕ ПРОШЛИ!\n');
  console.log('Проверьте реализацию:');
  console.log('   - Таблица riffleWeightsPerM2 должна быть правильно заполнена');
  console.log('   - Формула должна быть: weightPerM2 × area / 1000');
  console.log('   - Проверьте, что используется правильное поле из базы данных');
}

console.log('\n═══════════════════════════════════════════════════════════════');

// Возвращаем код ошибки если тесты не прошли
process.exit(allTestsPassed ? 0 : 1);
